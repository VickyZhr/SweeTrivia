...
const CandySelectionScreen = () => {
  ...
  // Correctly prioritize remainingPoints → finalScore → context score
  const score =
    location.state?.remainingPoints ??
  ...
  const handleSelectCandy = (candyType: string, requiredPoints: number) => {
    ...
    navigate('/dispensing', {
      state: {
        candyType,
        remainingPoints: score - requiredPoints,
      },
    });
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden" style={{ backgroundColor: '#E0178C' }}>
      ...
      <div className="w-full max-w-4xl z-10 mt-0">
        ...
        <div className="grid grid-cols-2 gap-6 mx-auto max-w-2xl mb-16">
          <div onClick={() => handleSelectCandy('circle', 10)} className="bg-yellow-300 hover:bg-yellow-400 text-black p-6 rounded-xl border-4 border-white h-36 flex flex-col items-center justify-center cursor-pointer">
            <Circle className="w-20 h-20 mb-2 stroke-[3px]" />
            <span className="font-mono text-4xl font-bold">10 points</span>
          </div>
          <div onClick={() => handleSelectCandy('triangle', 20)} className="bg-yellow-300 hover:bg-yellow-400 text-black p-6 rounded-xl border-4 border-white h-36 flex flex-col items-center justify-center cursor-pointer">
            <Triangle className="w-20 h-20 mb-2 stroke-[3px]" />
            <span className="font-mono text-4xl font-bold">20 points</span>
          </div>
          <div onClick={() => handleSelectCandy('square', 30)} className="bg-yellow-300 hover:bg-yellow-400 text-black p-6 rounded-xl border-4 border-white h-36 flex flex-col items-center justify-center cursor-pointer">
            <Square className="w-20 h-20 mb-2 stroke-[3px]" />
            <span className="font-mono text-4xl font-bold">30 points</span>
          </div>
          <div onClick={() => handleSelectCandy('star', 40)} className="bg-yellow-300 hover:bg-yellow-400 text-black p-6 rounded-xl border-4 border-white h-36 flex flex-col items-center justify-center cursor-pointer">
